...
export function getProductInputValues(
  ...
  return {
    ...
    categories: Array.isArray(categories) ? categories.map((category) => category?.id) : [],
    shop_id: Array.isArray(shop_id) ? shop_id.map((item) => item?.id) : [],
